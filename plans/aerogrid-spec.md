...
- A component may declare `validateSettings(settings)`, returning messages, for a rule that spans more than one setting. `choices` catches a value that is wrong on its own; this catches a pair that is wrong together, which is the only authoring mistake the per-setting schema cannot see.
