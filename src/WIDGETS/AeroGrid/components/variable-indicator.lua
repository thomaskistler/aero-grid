...
function variableIndicator.read(context)
    ...
    if type(feed) == "table" then
        ...
        if type(feed.min) == "number" and type(feed.max) == "number" and feed.max ~= feed.min then
            out.min = feed.min
            out.max = feed.max
        end
    end
    ...
    if type(settings.rangeMin) == "number" then
        out.min = settings.rangeMin
    end
    if type(settings.rangeMax) == "number" then
        out.max = settings.rangeMax
    end
...
function variableIndicator.create(parent, rect, settings, services)
    ...
    if settings.binding == "source" then
        ...
        if control then
            ...
            context.feed = control:globalVariable(settings.index, type(mode) == "number" and mode >= 0 and mode or nil)
        end
    end
    ...
    local area = variableIndicator.regionsFor(theme, services.themeBuilder, rect, layout, fonts, forms)
    ...
    context.label, context.badge = primitives.header(
        ...
        area.frame,
        fonts,
        variableIndicator.labelText(context, reading),
        presentation,
...
function variableIndicator.hideVisual(context)
    if context.bar then
        ...
        if context.bar.marker then
            lvgl.hide(context.bar.marker)
        end
    end
...
function variableIndicator.showVisual(context)
    if context.bar then
        ...
        if context.bar.markerFraction then
            lvgl.show(context.bar.marker)
        end
    end
    if context.bipolar then
...
function variableIndicator.render(context, out)
    ...
    if context.showDetail and settings.showName and reading.name ~= "" then
        ...
        if
            settings.binding ~= "source"
            and type(context.feed) == "table"
            and type(context.feed.flightMode) == "number"
        then
    ...
    if context.showVisual then
        out.fraction = variableIndicator.fraction(context.visualName, reading, context.primitives.signedFraction)
        -- The zero tick only means something when the range actually spans it.
        out.marker = variableIndicator.crossesZero(reading) and (-reading.min / (reading.max - reading.min)) or nil
    end
end
...
function variableIndicator.apply(context, drawn)
    ...
    if unitText ~= context.unitText then
        ...
        local shows = context.primitives.unitFits(
    ...
    if not context.showVisual then
        return
    end
...
function variableIndicator.refresh(context)
    ...
    local changed, drawn = context.primitives.changed(context, variableIndicator.render)
    if changed then
        variableIndicator.apply(context, drawn)
    end
end
...
function variableIndicator.update(context, rect)
    ...
    local moved = area.showDetail ~= context.showDetail or area.showVisual ~= context.showVisual
    if moved then
    ...
    local reading = variableIndicator.read(context)
    local fraction = variableIndicator.fraction(context.visualName, reading, primitives.signedFraction)

    if context.bar then
        primitives.placeBar(context.bar, area.pad, area.barY, area.content, fraction)
    end
    if context.bipolar then
        primitives.placeBipolarBar(context.bipolar, area.pad, area.barY, area.content, context.bipolar.h, fraction)
    end
    if context.radial then
        primitives.placeRadial(context.radial, area.visualCentreX, area.visualCentreY, area.radius)
    end
