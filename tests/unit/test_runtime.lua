...
local function testTimerSemantics()
    ...
    -- supporting row.
    local countingUp = { available = true, countdown = false, elapsed = 64, value = 64, start = 0 }
    assertEqual(
        timer.detailVariants(countingUp, tostring)[1],
        "COUNTING UP",
    ...
    for _, reading in ipairs({ "model", "elapsed" }) do
        assertEqual(
            timer.detailVariants(countingUp, tostring, { reading = reading })[1],
            "COUNTING UP",
    ...
    local countdown =
        { available = true, countdown = true, start = 300, remaining = 90, elapsed = 210, expired = false }
    assertEqual(
        timer.detailVariants(countdown, tostring, { reading = "remaining" })[1],
        "OF 300",
        "a countdown asked for its remaining was told it has none"
    )
    ...
    for _, case in ipairs({
        ...
        {
            "elapsed past zero",
            { available = true, countdown = true, start = 300, remaining = -15, elapsed = 315, expired = true },
            nil,
        },
        ...
        assert(
            width <= 86,
            string.format(
                "flight-timer: %s draws %q at %d px, past the 86 px a row sharing a" .. " line is given",
                case[1],
                variants[1],
                width
            )
    ...
    assertEqual(
        #timer.validateSettings({}, nil, { timer = 3 }),
        1,
        "the first index past the last real timer was accepted"
    )
    assertEqual(#timer.validateSettings({}, nil, { timer = -1 }), 1)
    assertEqual(#timer.validateSettings({}, nil, { timer = 1.5 }), 1, "a fractional index was accepted")
...
local function testTxBatteryEstimate()
    ...
    assertEqual(battery.hasRange({ packEmpty = 8.4, packFull = 6.6 }), false, "an inverted range is not a range")
...
local function testVariableNormalization()
    ...
    assertEqual(indicator.fraction("bar", range, signed), 1, "the drawing is clamped")
    assertEqual(indicator.format(range.value, 0), "150", "the value is not clamped")

    local bipolar = { value = -25, min = -50, max = 100 }
    assertEqual(indicator.fraction("bipolar-bar", bipolar, signed), -0.5)
    assertEqual(indicator.fraction("bar", { value = 25, min = 0, max = 100 }, signed), 0.25)
    assertEqual(indicator.fraction("radial", { value = nil, min = 0, max = 100 }, signed), 0)
    assertEqual(
        indicator.fraction("bar", { value = 5, min = 5, max = 5 }, signed),
        0,
        "a collapsed range cannot produce a fraction"
    )
...
local function testTrimPresentation()
    ...
    local right =
        { available = true, raw = 240, value = 30, fraction = 0.234375, centered = false, threePosition = false }
    ...
    local centred = { available = true, raw = 0, value = 0, fraction = 0, centered = true, threePosition = false }
    ...
    local toggle = { available = true, raw = 1024, value = 128, fraction = 1, centered = false, threePosition = true }
    ...
    assertEqual(trims.isVertical({ orientation = "vertical", orientation2 = "horizontal" }, 2, wide), false)
end
...
local function testCellShapes()
    ...
    local long = {}
    for index = 1, cellBattery.CELL_LIMIT + 8 do
        long[index] = 4.0
    end
...
local function testCellReadings()
    ...
    assertEqual(cellBattery.fraction(pack, 15.0, 0), 0, "a pack with no known cell count cannot be scaled")
    ...
    local function count(summary, width)
        return theme.fitLabel(cellBattery.countVariants(summary, { showCount = true }), SMLSIZE, width)
    end
    ...
    for _, case in ipairs({ { "number", "CELLS ERR" }, { "invalid", "CELLS ERR" }, { "empty", "NO CELLS" } }) do
        local variants = cellBattery.countVariants({ shape = case[1] }, { showCount = true })
        assertEqual(#variants, 1, case[1] .. " offers a ladder it cannot use")
        local width = theme.measureText(SMLSIZE, variants[1])
        assert(
            width <= 86,
            string.format(
                "%s draws %q at %d px, past the 86 px" .. " a row sharing a line is given",
                case[1],
                variants[1],
                width
            )
    ...
    assert(
        count({ shape = "empty" }, 400) ~= count({ shape = "number" }, 400),
    ...
    assertEqual(
        theme.fitLabel(cellBattery.countVariants(short, { showCount = true, cells = 4 }), SMLSIZE, 400),
        "3S OF 4"
    )
    assertEqual(theme.fitLabel(cellBattery.countVariants(short, { showCount = true, cells = 4 }), SMLSIZE, 50), "3S/4")

    -- The pack row shortens the same way, and says nothing when switched off.
    assertEqual(
        theme.fitLabel(cellBattery.packVariants({ pack = 16.4 }, { showPack = true }), SMLSIZE, 400),
        "16.4V PACK"
    )
    assertEqual(theme.fitLabel(cellBattery.packVariants({ pack = 16.4 }, { showPack = true }), SMLSIZE, 60), "16.4V")
    assertEqual(theme.fitLabel(cellBattery.packVariants({ pack = 16.4 }, { showPack = false }), SMLSIZE, 400), "")
end
...
local function testLinkClassification()
    ...
    assertEqual(linkStatus.classify({ name = "RSSI", known = true, available = true, stale = true }), "stale")
    assertEqual(linkStatus.classify({ name = "RSSI", known = true, available = true }), "live")
    ...
    assertEqual(linkStatus.primaryFor({ reading = "quality" }, "live", "absent"), "quality")
    ...
    assertEqual(link({ sourceState = "absent", indicator = true, rssiState = "absent" }), "NO RSSI SENSOR")

    -- A dead link is the measurement this panel exists to report.
    assertEqual(state({ sourceState = "live", linkDown = true, available = true, value = 96 }), "critical")
    assertEqual(link({ linkDown = true }), "LINK DOWN")

    -- Never powered up is not an alarm.
    assertEqual(state({ sourceState = "waiting", linkDown = true, available = false }), "unavailable")
    ...
    assertEqual(link({ sourceState = "absent", indicator = true, rssiState = "absent" }, 90), "NO RSSI")
    ...
    assertEqual(linkStatus.sourceText({ value = 78, precision = 0, unitText = "dB" }, "live"), "78dB")
    assertEqual(linkStatus.sourceText({ value = -72.4, precision = 1, unitText = "dBm" }, "live"), "-72.4dBm")
...
local function testNavigationPresentation()
    ...
    local noFix = { source = "GPS", known = true, fix = false, home = false, state = "unavailable" }
    ...
    assertEqual(
        navigation.resolveState({}, { source = "GPS", known = false, fix = false, home = false }),
        "unavailable"
    )
    assertEqual(navigation.originText({ source = "GPS", known = false }), "NO GPS SOURCE")
    ...
    local stale =
        { source = "GPS", known = true, fix = true, home = true, state = "stale", distance = 778, bearing = 9.47 }
