...
function flightMode.validateSettings(settings, span, config)
    ...
    if not (config and config.showIndex) then
        return messages
    end
...
---@return table
function flightMode.regionsFor(theme, themeBuilder, rect, layout, fonts, widest, out)
...
function flightMode.create(parent, rect, settings, services)
    ...
    local layout = flightMode.presentationFor(span.colSpan, span.rowSpan, settings.showIndex)
    ...
    local area = flightMode.regionsFor(theme, services.themeBuilder, rect, layout, fonts, widest)
...
function flightMode.apply(context, drawn)
    ...
    context.primitives.centreReading(context, context.themeBuilder, context.area, context.area.name, drawn.text)
...
function flightMode.update(context, rect)
    ...
    context.primitives.centreReading(context, context.themeBuilder, area, area.name, context.text)
