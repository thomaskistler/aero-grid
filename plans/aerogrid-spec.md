...
- The semantic accent is a plain stripe on the panel's left edge: constant width, square ends, cut off flat where the panel's corner curves away. It does not taper and it does not wrap around the corner.
- Its vertical inset is the corner radius, and that is forced rather than chosen. A square-ended stripe occupies `x` from 0 to the accent width, and can only sit inside the panel where the panel's own left boundary has reached `x = 0`, which for a corner radius `R` is true only at `y >= R`. Above that the fill has curved away and a stripe pixel would be drawn outside the card. Rasterising a 4 px stripe against a rounded panel puts the first fully enclosed row at exactly `R`, at every panel size.
- The only lever on the stripe's length is therefore the corner radius. At 480 x 272 a one-row panel is 65 px tall, and a radius of 8 leaves the stripe 49 px, 6 leaves 53, and 4 leaves 57. A longer stripe means a less rounded card; there is no separate inset to tune, and inventing one lets the stripe escape the panel.
- Two constructions were tried and rejected on a screen before this one. A stripe with `rounded` set to its own width gains semicircular caps and tapers at both ends. Making the accent the panel's base and insetting the surface over it produces a band that follows the corner arc round instead of stopping, sliding right as it goes.
...
- The simulator fixture carries six screens, every one holding an AeroGrid instance: `sim`, which fills its grid with the telemetry components, `sim2`, which covers the radio-local ones that had nowhere to go beside them, and the four span galleries. A gallery only ships as a layout, and selecting a layout means setting the widget's Dashboard ID, which in App mode cannot be reached from the main view at all: `Widget::openMenu` returns immediately after `setFullscreen(true)` when the widget is not in the top bar and the view is App mode. Without a screen apiece, reaching a gallery means going through Model Setup and Screens once per gallery. `MAX_CUSTOM_SCREENS` is 10, so six leaves room. Paging between them switches dashboards without opening widget settings, and exercises two widget instances resolving different layouts at once. `sim` carries the Modern palette and `sim2` the EdgeTX-derived one, so the two are one button press apart.
...
One thing the rule cannot currently reach: **LVGL's own behaviour is not citable from this checkout.** `radio/src/thirdparty/lvgl` is a submodule and it is not initialised, so `lv_draw_rect` and everything else below EdgeTX's own wrappers cannot be read. Anything the dashboard depends on from inside LVGL is therefore an unverifiable claim, and the rule's third part applies to it with particular force: a comment asserting what LVGL does is a claim nobody here has checked. One shipped example was found and removed during the presentation pass, where a panel's accent was built as a pill on the strength of a comment asserting that LVGL clamps a corner radius to half the shorter side. The replacement construction was chosen partly because it does not depend on that behaviour at all. Prefer a construction that is indifferent to what LVGL does over one that relies on an assertion about it, and initialise the submodule before writing either.
