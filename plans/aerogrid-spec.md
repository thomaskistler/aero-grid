...
The second reserved a band. `theme.ladder` grants a supporting row from the panel's height and passes that grant to `theme.bands`, which takes the tertiary quarter out of the body. Three components default their row off -- `flight-mode`'s mode number, `model-identity`'s label list, `tx-battery`'s estimate -- so all three were charged a quarter of the panel for a row they would never fill, and their readings came from a band 31 pixels shorter than the panel had. Each lost a font size at every two-row span, and `flight-mode` additionally built a label and wrote an empty string into it, which is the invisible work this document forbids, in a component that had already been swept for exactly that.
