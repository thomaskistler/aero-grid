...
function trimPanel.isVertical(settings, index, rect)
    ...
    if override == "horizontal" then
        return false
    end

    local panel = settings.orientation
    if panel == "vertical" then
        return true
    end
...
function trimPanel.regionsFor(theme, themeBuilder, rect, count, fonts)
    ...
    local showCaption = cellHeight >= labelHeight * 2 + BAR_THICKNESS + 4 and cellWidth >= MIN_TEXT_CELL
    local showValue = cellHeight >= labelHeight + BAR_THICKNESS + 4 and cellWidth >= MIN_TEXT_CELL
...
function trimPanel.cellFor(area, index, vertical)
    ...
    if barRoom < 2 then
        barRoom = math.max(2, area.cellHeight)
    end
