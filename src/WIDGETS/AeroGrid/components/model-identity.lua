...
function modelIdentity.presentationFor(settings, colSpan, rowSpan)
    ...
    if choice == "image" then
        return { showName = false, showImage = true }
    end
    if choice == "both" then
        return { showName = true, showImage = true }
    end
...
function modelIdentity.validateSettings(settings, span, config)
    ...
    if config.label ~= nil and (config.presentation == "image" or config.presentation == "both") then
...
function modelIdentity.regionsFor(theme, themeBuilder, rect, layout, fonts)
    ...
    local nameFont, formIndex = themeBuilder.fitReading(FORMS, frame.content, ladder.room)
    ...
    if showLabels then
        imageHeight = imageHeight - labelHeight - 2
    end
...
function modelIdentity.create(parent, rect, settings, services)
    ...
    local layout = modelIdentity.presentationFor(settings, span.colSpan, span.rowSpan)
    layout.showLabels = settings.showLabels == true
    local presentation = services.state("normal", settings.accent)
    local area = modelIdentity.regionsFor(theme, services.themeBuilder, rect, layout, fonts)
...
function modelIdentity.ensureImage(context, path)
    if context.image or not context.area.showImage then
        return
    end
    ...
    if not context.layout.showName then
        lvgl.hide(context.value)
    end
end
...
function modelIdentity.apply(context, drawn)
    ...
    context.primitives.centreReading(context, context.themeBuilder, context.area, context.area.nameFont, drawn.text)
    ...
    if context.showLabels then
        ...
        context.primitives.centreLabel(
            ...
            context.themeBuilder,
            context.labelsLabel,
            context.area.labelsCentre,
            context.area.labelsY,
            context.fonts.label,
            context.labelsText
        )
    end
...
function modelIdentity.resolveImage(context, drawn)
    if drawn.state ~= "normal" or not context.area.showImage then
        return
    end
    ...
    if context.imagePath == path or context.imageRejected == path then
        return
    end
...
function modelIdentity.revealName(context)
    ...
    local area = context.area
    context.value:set({ x = area.valueX, y = area.nameY, w = area.valueWidth })
...
function modelIdentity.refresh(context)
    ...
    local changed, drawn = context.primitives.changed(context, modelIdentity.render)
    if changed then
        modelIdentity.apply(context, drawn)
    end
end
...
function modelIdentity.update(context, rect)
    ...
    primitives.centreReading(context, context.themeBuilder, area, area.nameFont, context.text)
    ...
    primitives.centreLabel(
        ...
        "labelsAnchor",
        context.themeBuilder,
        area.showLabels and context.labelsLabel or nil,
        area.labelsCentre,
        area.labelsY,
        context.fonts.label,
        context.labelsText
    ...
    if area.showLabels ~= context.showLabels then
