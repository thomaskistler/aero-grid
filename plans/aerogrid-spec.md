...
- A supporting row cannot shrink its font, because it is already at the smallest size the dashboard uses. **The property is that a row fits the box its panel gives it**; the mechanism, where a row would not, is to offer several wordings longest first and let `theme.fitLabel` take the longest that fits. `testSupportingRowsFitTheirBox` holds the property at every span that grants a row, in both zones, with each component driven into the state that words the most.

  **Corrected: this said every supporting row in the catalogue goes through `fitLabel`, which was never true and was the wrong thing to require.** Five of the eight components that draw a row call it. Routing the other three through it would change nothing, because a single-wording list comes back out of `fitLabel` unchanged -- and all three fit at the widest string they can print, measured: `flight-mode`'s `#9` at 15 px, `metric`'s `VS 888.8m/s` at 75, `variable-indicator`'s `RATES FM8` at 68, against the 105 px box of the narrowest panel that draws a row.

  Stating the mechanism as the rule hid the one row that actually broke it. `flight-timer` offered one wording per state with no shorter form, and `ELAPSED PAST ZERO` is 125 px into that same 105 px box -- so it was centred on a box wider than its panel and ran ten pixels off each edge. Nothing caught it, because nothing was checking the property: the suite checked which components called the helper, and that component did not, so there was no rule for it to be seen breaking.
