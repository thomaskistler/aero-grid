...
- A component whose reading holds no redundancy offers exactly one form, and that is how it says so. `metric` draws its unit as a separate label, `flight-timer` draws a clock, and `navigation` draws a distance whose unit changes with range; none of the three can shorten anything.
...
`REFLOW_BATCH` and the reveal frame were the recommended items here and both have been taken; see [Why `REFLOW_BATCH` is three](#why-reflow_batch-is-three) and the render-declaration entry under fixture discipline. `primitives.arcBounds` is gone with them. What is left is the two questions that are decisions rather than work — `link-status`'s runtime-varying threshold unit, and whether the three development components ship — and the physical readability review, which needs hardware.
