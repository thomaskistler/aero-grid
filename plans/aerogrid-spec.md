...
- The semantic accent traces the panel's whole left edge at constant thickness: a straight run between the corners, and a quarter-circle band at each corner carrying it round to the tangent. It is the panel's own edge thickened inward, so it neither tapers nor stops short.
- It is built as three objects, because a single rectangle cannot do it. A stripe with `rounded` set gains semicircular caps and tapers; a full-height square stripe protrudes past the corner arc; and making the accent the panel's base with the surface inset over it produces a band whose position drifts through the corner, because it is the difference between two offset rounded rectangles rather than an arc. The straight run is a plain rectangle from `y = radius` to `y = height - radius`, and each corner is an `lvgl.arc` centred on the panel's corner centre, with the panel's corner radius and a thickness of the accent width.
- That the two curves coincide follows from the firmware rather than from tuning. LVGL draws an arc band between `radius` and `radius - width` measured from the centre (`lv_draw_arc.c`, `rout` and `rin`), and the object's box is `2 * radius` square about the position EdgeTX was given (`LvglWidgetArc::build` calls `setRadius`; `get_center` in `lv_arc.c` takes `min(w, h) / 2`). Passing the panel's corner centre and corner radius therefore puts the band's outer edge on the same circle the panel's fill is rounded by. Rasterising confirms it: the accent's outer edge equals the panel's outer edge on every row, no row is left bare, and no accent pixel falls outside the panel.
- Corner bands are arcs, so hard-won constraint 11 applies to them. Every colour change is a `set`, and a `set` on a round object walks it up and left by its own radius unless the centre is restated, so a panel that changed state repeatedly would march its own corners off the screen. Both bands go through the shared `setRound` helper.
