...
local function testRendersInBothModes(label, zone, path, expected)
    ...
    for _, entry in ipairs(context.components) do
        ...
        assert(
            panel.w <= bounds.w and panel.h <= bounds.h,
...
local function testSessionArmsTheFlight()
    ...
    local function flightOf(yaml)
        ...
        local context = createLoaded({ x = 0, y = 0, w = 480, h = 272 }, DEFAULT_OPTIONS, widget)
    ...
    assertEqual(armed.configured, true, "the layout named an arm switch and the flight session did not see it")

    local unarmed = flightOf(dashboard("# no session block"))
    assertEqual(unarmed.configured, false, "a dashboard that names no arm switch reported one anyway")
end
    ...
    for _, stem in ipairs(stems) do
        ...
        if componentHost.supportsSpan(module, 1, 1) then
            ...
            assert(
                ...
                stem
                    .. " declares a 1 x 1 span but is missing from the 1 x 1 gallery,"
...
local function testStatesCoverBothPalettes()
    ...
    for _, mode in ipairs({ "modern", "edgetx" }) do
        ...
        assertEqual(critical.instance.stateName, "critical", mode .. ": the panel meant to be critical is not")

        local warning = entryById(context, "s-warning")
        assertEqual(warning.instance.stateName, "warning", mode .. ": the panel meant to be warning is not")

        -- And each is actually drawn on its tint, rather than merely resolving to
        -- a state that has one.
        assertEqual(
            critical.instance.panel.background.properties.color,
            context.theme.alertColor.critical,
            mode .. ": the critical panel is not drawn on the critical tint"
        )
        assertEqual(
            warning.instance.panel.background.properties.color,
...
local function testShippedLayout()
    ...
    assertEqual(pack.showDetail, false, "a 2 x 1 cell panel is tall enough for a supporting row after all")
    ...
    for _, row in ipairs({ { nav.detail, nav.detailWidth }, { nav.origin, nav.originWidth } }) do
        ...
        assert(
            needed <= row[2],
            string.format("the shipped navigation panel draws %q, needing %d px of %d", row[1], needed, row[2])
        )
    end
...
local function testBackgroundsArePainted()
    local function assertPainted(object, what)
        ...
        assert(
            type(color) == "number" and color % 65536 == 0x8000,
...
local function testPanelPresentation()
    ...
    assert(
        themeModule.contrast(modern.canvas, modern.surface) >= 1.30,
    ...
    for _, entry in ipairs(appContext.components) do
        ...
        assertEqual(
            panel.background.painted.radius,
            spacing.radius,
        ...
        assertEqual(column.w, spacing.accentWidth, id .. " accent mask is not one accent width across")
        assertEqual(column.h, bounds.h, id .. " accent mask does not cover the panel's height")
        ...
        local function parentName(object)
            if object.parent == panel.column then
                return "the accent mask"
            end
            if object.parent == panel.root then
                return "the panel root"
            end
            return "something else"
        end
        for what, object in pairs({
            ["accent stripe"] = panel.accent,
            ["top band"] = panel.topArc.arc,
            ["bottom band"] = panel.bottomArc.arc,
        }) do
            assert(
                object.parent == panel.column,
                id
                    .. ": the "
                    .. what
                    .. " is parented to "
        ...
        assert(
            spacing.radius * 2 > spacing.accentWidth,
            id
                .. " the corner bands fit inside the accent width unclipped, so the"
        ...
        assertEqual(panel.accent.painted.radius, 0, id .. " accent was drawn with rounded ends")
        ...
        assertEqual(accent.y, spacing.radius, id .. " accent does not start at the top corner's tangent")
        assertEqual(
            accent.y + accent.h,
            bounds.h - spacing.radius,
        ...
        for corner, band in pairs({ top = panel.topArc, bottom = panel.bottomArc }) do
            ...
            assertEqual(band.centreX, spacing.radius, id .. " " .. corner .. " band is not on the corner centre")
            ...
            assertEqual(
                band.arc.round.drawn.x,
                band.centreX - spacing.radius,
                id .. " " .. corner .. " band drifted horizontally"
            )
            assertEqual(
                band.arc.round.drawn.y,
                band.centreY - spacing.radius,
                id .. " " .. corner .. " band drifted vertically"
            )
        end

        assertEqual(panel.topArc.centreY, spacing.radius, id .. " top band is not on the top corner centre")
        assertEqual(
            panel.bottomArc.centreY,
            bounds.h - spacing.radius,
        ...
        assertEqual(panel.topArc.arc.properties.startAngle, 180, id .. " top band does not start at nine o'clock")
        assertEqual(panel.topArc.arc.properties.endAngle, 270, id .. " top band does not end at twelve o'clock")
        assertEqual(panel.bottomArc.arc.properties.startAngle, 90, id .. " bottom band does not start at six o'clock")
        assertEqual(panel.bottomArc.arc.properties.endAngle, 180, id .. " bottom band does not end at nine o'clock")
        ...
        for _, object in ipairs(lvglMock.objects) do
            if object.parent == panel.root and object.kind == "label" and not object.hidden then
                labels = labels + 1
                assert(
                    object.properties.x >= spacing.accentWidth + spacing.accentGap,
                    id .. " drew text against the accent, at x " .. tostring(object.properties.x)
                )
            end
        end
  ...
  - id: pack
    ...
    local function assertLegible(name, tint)
        ...
        for _, check in ipairs(checks) do
            ...
            assert(
                ratio >= check[3],
                string.format("the %s tint leaves %s at %.2f, below %.1f", name, check[1], ratio, check[3])
            )
        end
        ...
        assert(
            elevation >= 1.30,
            string.format("the %s tint leaves the panel flat against the screen at %.2f", name, elevation)
        )
        ...
        assert(
            separation >= 1.30,
            string.format("the %s tint is indistinguishable from a resting panel at %.2f", name, separation)
        )
    end
    ...
    for name, accent in pairs(accents) do
        ...
        assert(ratio >= 2.5, string.format("the %s accent is lost on its own tint at %.2f", name, ratio))
    end

    metricModule.setValue(entry.instance, 20.5)
    assertEqual(entry.instance.stateName, "warning")
    assertEqual(panel.background.properties.color, resolved.alertColor.warning, "a warning did not tint the panel")
    ...
    assertEqual(panel.background.properties.color, resting, "stale data tinted the panel")

    metricModule.setValue(entry.instance, nil)
    assertEqual(entry.instance.stateName, "unavailable")
    assertEqual(panel.background.properties.color, resting, "a missing source tinted the panel")
    ...
    assertEqual(panel.background.properties.color, resting, "a panel stayed tinted after its reading recovered")
end
...
components:
  - id: pack
    ...
    assertEqual(
        panel.border.properties.color,
        context.theme.color.cyan,
        "a selection outline was not drawn in the focus colour"
    )
    assertEqual(
        panel.border.painted.borderWidth,
        spacing.borderFocus,
        "a selection outline was not drawn at the focus weight"
    )

    local bounds = boundsOf(entry)
    assertEqual(panel.border.properties.w, bounds.w, "a revealed outline kept the width it had before the reflow")
    assertEqual(panel.border.properties.h, bounds.h, "a revealed outline kept the height it had before the reflow")
...
components:
  - id: pack
    ...
    local function assertAccent(expected, what)
        ...
        assertEqual(panel.bottomArc.arc.properties.color, expected, what .. ": bottom band")
        for corner, band in pairs({ top = panel.topArc, bottom = panel.bottomArc }) do
            assertEqual(
                band.arc.round.drawn.x,
                band.centreX - spacing.radius,
    ...
    local bounds = boundsOf(entry)
    assertEqual(panel.topArc.centreY, spacing.radius, "the top band moved during a reflow")
    assertEqual(panel.bottomArc.centreY, bounds.h - spacing.radius, "the bottom band did not follow the panel's height")
    ...
    assertEqual(panel.column.properties.h, bounds.h, "the accent mask did not follow the panel's height")
    assertEqual(panel.column.properties.w, spacing.accentWidth, "the accent mask changed width during a reflow")
...
components:
  - id: probe
    ...
    local bounds = boundsOf(entry)
    local frame = themeModule.frame(context.theme, { x = 0, y = 0, w = bounds.w, h = bounds.h }, probe.fonts)
    ...
    assertEqual(title.x, frame.labelX, "the probe's title is not where the shared frame puts a label")
    assertEqual(title.w, frame.labelWidth, "the probe's title is not the width the shared frame leaves a label")
    assertEqual(badge.x, frame.badgeX, "the probe's badge is not in the shared frame's badge column")
    assertEqual(badge.w, frame.badgeWidth, "the probe's badge is not the width of the shared frame's column")
    ...
    assert(title.x + title.w <= badge.x, "the probe's title runs into its own badge column")
...
components:
  ...
  - id: notcells
    ...
    local function assertRow(instance, text, width, expected, what)
        ...
        assert(needed <= width, string.format("%s needs %d px of %d", what, needed, width))
    end
    ...
    -- to it.
    assertRow(link, link.linkDetail, link.rowRightWidth, "78dB", "a healthy link names its secondary source")
    assertRow(pack, pack.countText, pack.detailWidth, "4S", "the cell count")
    assertRow(pack, pack.packText, pack.detailWidth, "16.4V", "the pack sum")
    ...
    assertRow(wrong, wrong.countText, wrong.detailWidth, "CELLS ERR", "a plain number from a cells source")
    ...
    assertRow(pack, pack.countText, pack.detailWidth, "CELLS ERR", "nonsense from a cells source")
    ...
    assertRow(link, link.linkDetail, link.rowRightWidth, "NO LINK", "a dead link says so in words")
...
local function testReadingsAgreeAcrossComponents()
    ...
    local function fontsAt(colSpan, rowSpan, entries)
        ...
        for index, entry in ipairs(entries) do
            ...
            col = col + colSpan
            if col + colSpan > 4 then
                col = 0
                row = row + rowSpan
            end
        end

        local widget = makeWidget("agree-" .. colSpan .. "x" .. rowSpan, table.concat(lines, "\n") .. "\n")
        resetRadio()
        local context = createLoaded({ x = 0, y = 0, w = 480, h = 272 }, DEFAULT_OPTIONS, widget)
        pump(context, 40)
        assertEqual(#context.errors, 0, table.concat(context.errors, "\n"))
    ...
    for _, span in ipairs({ { 1, 1 }, { 2, 1 }, { 4, 1 }, { 2, 2 } }) do
        ...
        assert(high - low <= 1, string.format("%s: readings on identical panels span %d font steps", name, high - low))
    end
    ...
    local agreed = 0
    for _, value in pairs(spread) do
        if value == 0 then
            agreed = agreed + 1
        end
    end
    assert(
        agreed >= 2,
        "no span reached full agreement, which would be the"
...
local function testMetricStates()
    ...
    assertEqual(
        pack.panel.accent.properties.color,
        lcd.RGB(modern.cyan),
    ...
    assert(
        math.abs(readingCentre(pack) - before) <= 1,
        string.format(
...
local function testBadgeGeometry()
    ...
    assert(
        label.x + label.w <= badge.x,
        "badge overlaps the label: label ends at " .. (label.x + label.w) .. ", badge starts at " .. badge.x
    )
...
components:
  - id: dial
    ...
    assert(resized.properties.radius < firstRadius, "radial did not shrink with the panel")
...
components:
  - id: dial
    ...
    -- draws cannot drift.
    assertEqual(dial.showVisual, true, "the panel shed its dial, so a drift test proves nothing")
    ...
    -- no longer 158".
    assertEqual(arc.properties.endAngle, 248, "the dial never moved, so a drift test proves nothing")
...
return exploder
    ...
    assert(
        not underButton(label.properties, text),
    ...
    assert(
        label.properties.y + themeModule.fontHeight(SMLSIZE) <= 272,
    ...
    assertEqual(assert(plain.errorLabel).properties.y, 8, "the overlay gave up room it did not have to")
    ...
    assertEqual(
        assert(wide.reserved, "no corner reserved on a wider display").h,
        wideButton,
        "the button height was assumed rather than read"
    )
    assertEqual(wide.reserved.w, 65, "the width kept clear does not match MENU_HEADER_BUTTONS_LEFT")
    ...
    assertEqual(moving.errorLabel.properties.y, 8, "the overlay did not follow the zone out from under the button")
    lvglMock.setAppMode(false)
end
...
local function testSupportingWordingsStayDistinct()
    ...
    local WORDED = {
        ...
        },
        {
            ...
            states = {
                ...
                { "no RSSI sensor exists", { "NO RSSI SENSOR", "NO RSSI SENSE", "NO RSSI", "NO RSS" } },
            },
        },
    }
    ...
    for _, subject in ipairs(WORDED) do
        ...
        for _, entry in ipairs(subject.states) do
            ...
            assert(
                type(shortest) == "string" and shortest ~= "",
    ...
    for _, entry in ipairs(WORDED[1].states) do
        ...
        local drawn = navigationModule.originText(entry[2], themeModule, fonts.label, area.originWidth)
        if drawn ~= full then
    ...
    assert(
        shortened > 0,
        "no state was shortened at the narrowest row, so the"
    ...
    local function walk(object, offsetX, offsetY, clip)
        for _, child in ipairs(object.children) do
            ...
            if child.properties.w and child.properties.h then
                inner = clipTo({ x = x, y = y, w = child.properties.w, h = child.properties.h }, clip)
            end
            if not child.hidden then
                ...
                if child.kind == "label" and text ~= "" then
                    ...
                    found[#found + 1] = clipTo({
                        ...
                        w = math.min(themeModule.measureText(size, text), child.properties.w or math.huge),
                        ...
                        h = themeModule.fontAscent(size) + edgetx.textDescent(size, text),
                    }, clip)
    ...
    for _, entry in ipairs(context.components) do
        ...
        for index, box in ipairs(boxes) do
            if not box.label then
                ...
                if spansPanel or inAccentColumn then
                    surface[index] = true
                end
            end
        end
        ...
        if rect then
            for _, box in ipairs(boxes) do
                if box.label then
                    assert(
                        box.y >= 0 and box.y + box.h <= rect.h,
                        where
                            .. " draws "
                            .. box.what
                            .. " off the panel vertically, "
                            .. box.y
                            .. " to "
                            .. (box.y + box.h)
                            .. " in "
...
local function testNothingIsDrawnOverAnythingElse()
    ...
    local function collect(directory, prefix)
        ...
        for name in listing:lines() do
            local stem = string.match(name, "^(.+)%.yaml$")
            if stem then
                names[#names + 1] = { stem = stem, path = directory .. name, label = prefix .. stem }
            end
        end
        listing:close()
        os.remove(listingPath)
    ...
    for _, entry in ipairs(names) do
        if string.match(entry.stem, "^span") then
    ...
    for _, entry in ipairs(names) do
        ...
        for _, mode in ipairs({ { "full screen", fullScreenZone }, { "app mode", appZone } }) do
            ...
            local widget = makeWidget("collide-" .. entry.stem .. "-" .. string.gsub(mode[1], " ", ""), yaml)
...
local function testTwoRowFooterClearsTheReading()
    for _, place in ipairs({ { "the menu button's corner", 0, 0 }, { "a clear cell", 2, 1 } }) do
        resetRadio()
        ...
        assertEqual(nav.showCoordinates, true, where .. " drew one supporting row, not two")
        ...
        assert(
            bottom <= rowTop,
            where .. ": the distance ends at " .. bottom .. " and the bearing row starts at " .. rowTop
        )
        -- And the rows clear each other, which is what the band has to be tall
        -- enough for in the first place.
        assert(
            rowTop < nav.coordinatesLabel.properties.y,
            where .. ": the coordinates do not sit below the bearing row"
        )
        ...
        assertEqual(
            nav.coordinatesLabel.properties.text,
            "47.37690 8.54170",
...
local function testTertiaryQuarterHoldsItsFurniture()
    ...
    local function occupants(entry, bounds)
        local fonts = themeModule.typography(entry.placement.colSpan, entry.placement.rowSpan)
        local panel = { x = 0, y = 0, w = bounds.w, h = bounds.h }
        local band = themeModule.ladder(
            entry.instance.theme,
            panel,
            themeModule.frame(entry.instance.theme, panel, fonts)
        ).bands.tertiary
            ...
            for _, child in ipairs(object.children) do
                ...
                if not child.hidden then
                    ...
                    elseif child.kind == "rectangle" or child.kind == "image" then
                        ...
                        local spansPanel = (child.properties.w or 0) >= bounds.w - 2 and height >= bounds.h - 2
                        local inAccent = x + (child.properties.w or 0) <= 8
                        if spansPanel or inAccent then
                            what = nil
                        end
                    end
                    if what and height and y + height > band.y and y < band.y + band.h then
    ...
    local CASES = {
        { name = "flight-mode bare", type = "flight-mode", config = { "label: MODE" }, row = false, bar = false },
        {
            name = "tx-battery bare",
            type = "tx-battery",
    ...
    for _, case in ipairs(CASES) do
        ...
        for _, span in ipairs({ { 2, 2 }, { 4, 2 } }) do
            for _, mode in ipairs({ { "full screen", fullScreenZone }, { "app mode", appZone } }) do
                ...
                local where = mode[1] .. " " .. case.name .. " " .. span[1] .. "x" .. span[2]
                ...
                if case.row then
                    ...
                    for _, box in ipairs(inside) do
                        assert(
                            box.top >= band.y and box.bottom <= band.y + band.h,
                            where
                                .. ": "
                                .. box.what
                                .. " runs from "
...
local function testTimerIndexIsRefusedAtLoad()
    ...
    local function load(value)
        ...
        return createLoaded({ x = 0, y = 0, w = 480, h = 272 }, DEFAULT_OPTIONS, widget)
    end
    ...
    assertEqual(#refused.errors, 1, "an index no radio has was accepted: " .. table.concat(refused.errors, "; "))
    assert(
        string.find(refused.errors[1], "clock", 1, true),
        "the refusal does not name the panel: " .. refused.errors[1]
    )
    assert(
        string.find(refused.errors[1], "timer", 1, true),
    ...
    assertEqual(#accepted.errors, 0, "the last timer a radio has was refused: " .. table.concat(accepted.errors, "; "))
    pump(accepted, 40)
    assertEqual(entryById(accepted, "clock").instance.text, "0:12", "timer 2 was accepted and then not read")
    ...
    assertEqual(#unconfigured.errors, 0, "a timer the model has not set up was reported as an authoring mistake")
...
local function testSupportingRowsFitTheirBox()
    ...
    local ROWED = {
        ...
        {
            name = "flight-timer, elapsed past zero",
            type = "flight-timer",
            config = { "timer: 0", "label: TIMER" },
            drive = function()
        ...
        { name = "flight-timer, counting up", type = "flight-timer", config = { "timer: 1", "label: TIMER" } },
        {
        ...
        { name = "flight-mode, mode number", type = "flight-mode", config = { "label: MODE", "showIndex: true" } },
        {
            name = "cell-battery, pack and count",
            type = "cell-battery",
            config = { "source: Cels", "label: PACK", "showPack: true", "showCount: true" },
        },
        {
            name = "cell-battery, nonsense cells",
            type = "cell-battery",
            config = { "source: Cels", "label: PACK", "showPack: true", "showCount: true" },
            drive = function()
                radio.values[130] = { 0, -1, 99 }
        ...
        {
            name = "link-status, link down",
            type = "link-status",
            config = { "label: LINK", "rssiSource: 1RSS", "qualitySource: RQly", "reading: rssi" },
            drive = function()
                radio.rssi = 0
            end,
        },
        ...
        {
            name = "navigation, no home position",
            type = "navigation",
            ...
            for _, mode in ipairs({ { "full screen", fullScreenZone }, { "app mode", appZone } }) do
                ...
                local function walk(object, offsetX, offsetY)
                    for _, child in ipairs(object.children) do
                        ...
                        local y = offsetY + (child.properties.y or 0)
                        local text = tostring(child.properties.text or "")
                        if child.kind == "label" and not child.hidden and text ~= "" and y > bounds.h / 2 then
                            local font = child.properties.font
                            font = type(font) == "function" and font() or font
                            ...
                            assert(
                                ...
                                where
                                    ...
                                    .. '" at '
                                    .. width
                                    .. " px into a content box of "
                                    .. frame.content
                                    .. " -- a row cannot shrink its font, so a"
                            ...
                            if width > widest then
                                widest, widestWhere = width, where
                            end
                        end
                        walk(child, x, y)
                    end
    ...
    assert(checked >= 60, "only " .. checked .. " supporting rows were drawn, so most of this swept nothing")
    assert(string.find(widestWhere, "1x2", 1, true) ~= nil or widest > 0, "the widest row found was not measured")
    resetRadio()
end
...
local function testBadgesEndFlushWithTheirPanel()
        ...
        for _, entry in ipairs(context.components) do
            ...
            if badge and not badge.hidden then
                ...
                if word ~= "" then
                    ...
                    assert(
                        badge.properties.x >= frame.badgeX,
                        where
                            .. ": "
                            .. entry.placement.id
                            .. " placed its badge at "
                            .. badge.properties.x
                            .. ", left of the column at "
                            .. frame.badgeX
    ...
    for _, mode in ipairs({ { "full screen", fullScreenZone }, { "app mode", appZone } }) do
        resetRadio()
        local widget = makeWidget("badge-flush-" .. string.gsub(mode[1], " ", ""), yaml)
        ...
        assert(
            moved,
            mode[1] .. ": no panel changed width, so the reflow below" .. " is not a reflow and proves nothing"
        )
    ...
    assert(checked >= 8, "only " .. checked .. " badged panels were found, so this proves very little")
    local reached = 0
    for word in pairs(seen) do
        assert(vocabulary[word], 'a panel drew "' .. word .. '", which is not in the badge vocabulary')
        reached = reached + 1
    end
    assert(reached >= 3, "only " .. reached .. " of the vocabulary was reached, so the narrow words may be untested")
end
...
components:
  - id: solo
    ...
    assertEqual(bounds.w, 92, "the cell is not the size this test was measured" .. " against")
    assertEqual(bounds.h, 44, "the cell is not the size this test was measured" .. " against")
    assert(area.frame.labelHidden, "this panel is wide enough for a heading, so it is not alone")
    assertEqual(instance.label.hidden, true, "a heading the frame dropped was drawn anyway")
    assertEqual(area.ladder.rows, 0, "this panel was granted a supporting row, so it is not alone")
    assertEqual(area.ladder.visual, false, "this panel was granted a visualization, so it is not alone")
    assertEqual(context.reserved, nil, "this panel sits under the menu button, so it is not alone")

    -- The budget is more than a half, which is the whole of the difference.
    assert(
        area.ladder.room > math.floor(bounds.h / 2),
        "a panel with nothing"
            .. " on it but its reading was still sized against a half: room "
            .. area.ladder.room
            .. " against "
            .. math.floor(bounds.h / 2)
    )
    ...
    assertEqual(
        edgetx.fontName(area.value),
        "MIDSIZE",
        "the whole-panel budget bought no size, so it is indistinguishable from" .. " the half"
    )
    assertEqual(
        edgetx.fontName(theme.bandFont(math.floor(bounds.h / 2))),
        "SMLSIZE",
    ...
    assert(math.abs((area.valueY + ink / 2) - bounds.h / 2) <= 1, "a reading alone on its panel is not centred on it")
    assert(
        area.valueY >= 0 and area.valueY + ink + theme.riderDepth() <= bounds.h,
...
components:
  - id: barred
    ...
    assert(
        barArea.ladder.room <= math.floor(barBounds.h / 2),
    ...
    assert(
        barArea.valueY + barInk + theme.riderDepth() <= barArea.barY,
        "the reading reaches "
            .. (barArea.valueY + barInk + theme.riderDepth())
            .. ", into a bar whose track starts at "
            .. barArea.barY
    )
    lvglMock.setAppMode(false)
end
...
components:
  ...
  - id: clear
    ...
    assertEqual(
        boundsOf(entryById(context, "corner")).h,
    ...
    assertEqual(
        context.reserved,
        nil,
        "one refresh after going fullscreen, a corner was still reserved for a" .. " button that is not drawn"
    )
    ...
    pump(context, 10)
    assertEqual(
        corner.area.ladder.room,
        clear.area.ladder.room,
    ...
    definition.refresh(context)
    assert(context.reserved, "one refresh after leaving fullscreen, the corner was not reserved again")
    passes = 0
    while context.reflowIndex do
        definition.refresh(context)
        passes = passes + 1
...
local function testReadingsAreCentredOnTheirPanel()
    ...
    local CONFIG = {
        ...
        ["link-status"] = { "      label: LINK", "      rssiSource: RSSI", "      qualitySource: RQly" },
        ...
        for _, span in ipairs({ { 1, 1 }, { 2, 1 }, { 2, 2 }, { 3, 2 }, { 4, 2 }, { 2, 3 }, { 4, 4 } }) do
            for _, place in ipairs(PLACES) do
                for _, zone in ipairs(ZONES) do
                    ...
                    if entry then
                        assertEqual(#context.errors, 0, where .. ": " .. table.concat(context.errors, "\n"))
                    end
                    ...
                    if area and area.ladder and area.value and area.valueY then
                        ...
                        if heading and not heading.hidden and tostring(heading.properties.text or "") ~= "" then
                            local font = heading.properties.font
                            if type(font) == "function" then
                                font = font()
                            end
                            local bottom = (heading.properties.y or 0) + theme.fontAscent(font)
                            cleared = cleared + 1
                            assert(
                                top >= bottom,
                                where
                        ...
                        if instance.showDetail and rowY then
                            ...
                            assert(
                                ...
                                where
                                    .. ": the reading reaches "
                                    .. (top + ink + depth)
                                    .. ", into a supporting row that starts at "
                                    .. rowY
                            )
                        end
                    end
...
local function testReadingsDoNotDescendOverAnything()
    ...
    assert(edgetx.textDescent(XXLSIZE, "rpm") > 0, "the fixture cannot see a descender, so nothing below is a check")
    assert(edgetx.textDescent(XXLSIZE, "km/h") > 0, "the fixture cannot see a solidus descend")
    for _, safe in ipairs({ "-888.88", "1:04:12", "120.0", "%", "V", "dBm" }) do
        assertEqual(
            edgetx.textDescent(XXLSIZE, safe),
            0,
            "the fixture reports a descender in " .. safe .. ", so it would report one anywhere"
        )
    end
    ...
    for _, unit in ipairs(DESCENDING_UNITS) do
        for _, span in ipairs(SPANS) do
            for _, mode in ipairs(ZONES) do
                ...
                local context = createLoaded(mode[2](), DEFAULT_OPTIONS, widget)
                pump(context, 60)
                assertEqual(#context.errors, 0, unit .. ": " .. table.concat(context.errors, "\n"))

                local panel = entryById(context, "subject").instance
                if panel.showUnit then
                    assertEqual(
                        panel.unit.properties.text,
                        unit,
                        "the unit the layout asked for never reached the label"
                    )
                    drewAUnit = drewAUnit + 1
                end
                assertNothingOverlaps(mode[1] .. " " .. unit .. " " .. span[1] .. "x" .. span[2], context)
            end
        end
    end
    ...
    for _, name in ipairs({ "Puppy", "Gypsy Moth", "jjjj", "Q_[g]" }) do
        for _, span in ipairs({ { 2, 2 }, { 4, 2 }, { 4, 4 } }) do
            for _, mode in ipairs(ZONES) do
                ...
                local context = createLoaded(mode[2](), DEFAULT_OPTIONS, widget)
                pump(context, 60)
                assertEqual(#context.errors, 0, name .. ": " .. table.concat(context.errors, "\n"))

                local panel = entryById(context, "subject").instance
                assertEqual(panel.text, name, "the hostile model name never reached the panel")
                drewAName = drewAName + 1
                assertNothingOverlaps(mode[1] .. " name " .. name .. " " .. span[1] .. "x" .. span[2], context)
            end
        end
    end
    ...
    assert(drewAUnit > 0, "every panel shed its unit, so no descending unit was ever drawn")
...
components:
  - id: tight
    ...
    local SWEEP_CONFIG = {
        ...
        ["link-status"] = { "      label: Probe", "      rssiSource: RSSI", "      qualitySource: RQly" },
            ...
            for rowSpan = 1, 4 do
                ...
                if not refused then
                    assertEqual(
                        #swept.errors,
                        0,
                        kind .. " " .. colSpan .. "x" .. rowSpan .. ": " .. table.concat(swept.errors, "\n")
                    )
...
return exploder
    ...
    for _, notice in ipairs(context.notices) do
        assert(
            notice.severity == "info" or notice.severity == "warning",
            "notice carried no usable severity: " .. tostring(notice.severity)
        )
        if string.match(notice.text, "corrected for contrast") then
            corrected = true
        end
    end
    ...
    assertEqual(
        #context.errors,
        0,
        "the legibility pass was reported as a failure: " .. table.concat(context.errors, "\n")
    )
...
components:
  - id: speed
    ...
    assert(alpha.serviceRuntime ~= beta.serviceRuntime, "two instances share a service registry")
    assert(
        alpha.serviceRuntime.byId.telemetry ~= beta.serviceRuntime.byId.telemetry,
...
local function testInstructionBudget()
    ...
    local function record(name, cost)
        ...
        if string.find(name, "reflow", 1, true) then
            if cost > worstReflow then
                worstReflow = cost
                worstReflowName = name
            end
    ...
    local function exercise(label, path, expectedComponents, expectedServices)
        ...
        for _, id in ipairs(expectedServices or {}) do
            ...
            assert(instance.count > 0, label .. ": nothing subscribed to the " .. id .. " service")
            assert(
                instance.revision > revisions[id],
    ...
    exercise(
        "service probes",
        makeWidget("budget-probes", probeGridLayout()),
        16,
...
return greedy
    ...
    assert(worstReflow > 0, "no reflow callback was measured, so the comparison below proves nothing")
    assert(worstOther > 0, "no other callback was measured")

    assert(
        worstReflow < worstOther,
        string.format(
    ...
    print(string.format("  budget headroom: worst callback %s used %d of %d", worstName, worst, BUDGET))
    print(string.format("  steady state:    worst frame %s used %d of %d", worstSteadyName, worstSteady, BUDGET))
    print(
        string.format(
            "  reflow:          worst %s used %d, against %s at %d",
            worstReflowName,
            worstReflow,
            worstOtherName,
            worstOther
...
components:
  - id: big
    ...
    assertEqual(instance.text, "3.0", "this test needs a panel with a reading, not a sentinel")
    ...
    assertEqual(
        instance.unit.hidden,
        false,
        "the unit was shed by a panel that only lost height, which it no longer" .. " pays for"
    )
    ...
    zone.h = 272
    settle()
    assertEqual(instance.unit.hidden, true, "a panel too narrow for the pair kept its unit")
    assert(instance.value.lines == 1, "the reading wrapped, so the unit was shed and bought nothing")
...
local function testRuntimeFailureIsContained()
    ...
    local context = definition.create({ x = 0, y = 0, w = 480, h = 272 }, DEFAULT_OPTIONS, widgetPath)
...
local function probeRows(instance)
    ...
    for index = 1, #instance.keys do
        ...
        if not key.hidden then
            ...
            if label ~= "" then
                rows[label] = instance.values[index].properties.text
            end
        end
    end
    return rows
...
local function testDiagnosticsFitTheirPanels()
    ...
    local function assertContained(what)
        for _, entry in ipairs(context.components) do
            ...
            for index = 1, #instance.keys do
                ...
                if not key.hidden then
                    assert(
                        key.properties.y + lineHeight <= bounds.h,
                        what .. ": " .. entry.placement.id .. " row " .. index .. " ran past its panel"
                    )
                    local value = instance.values[index].properties
                    assert(
                        value.x + value.w <= bounds.w,
                        what .. ": " .. entry.placement.id .. " row ran past the right edge"
                    )
                    assert(
                        key.properties.x + key.properties.w <= value.x,
    ...
    assert(entryById(context, "model").instance.visibleRows < tallest, "a shrunk panel did not shed rows")
    ...
    assertEqual(
        entryById(context, "model").instance.visibleRows,
        tallest,
    ...
    local function check(component, what, shown, key)
        ...
        assertEqual(
            ...
            when
                ...
                .. key
                .. (shown and " key" or " key it does not draw")
        )
    end
...
local function testReadingsIgnoreTheRowBeneathThem()
    ...
    local OPTIONAL = {
        ...
        },
        {
            ...
            config = { "label: MODEL" },
            label = function(panel)
                return panel.labelsLabel
            end,
            drew = function(panel)
                return panel.showLabels
            end,
        },
        {
            type = "tx-battery",
    ...
    for _, subject in ipairs(OPTIONAL) do
        ...
        for _, asked in ipairs({ false, true }) do
            ...
            local widgetPath =
                makeWidget("unused-" .. subject.type .. "-" .. tostring(asked), table.concat(lines, "\n") .. "\n")
            local context = createLoaded({ x = 0, y = 0, w = 480, h = 272 }, DEFAULT_OPTIONS, widgetPath)
            assertEqual(#context.errors, 0, table.concat(context.errors, "\n"))
            settle(context, 30)

            local panel = entryById(context, "panel").instance
            local where = subject.type .. " with " .. subject.setting .. " " .. tostring(asked)

            assertEqual(
                subject.drew(panel) == true,
                asked,
                where .. ": the panel disagrees with its own setting about whether it" .. " draws a supporting row"
            )
            ...
            if not asked then
                assertEqual(subject.label(panel), nil, where .. ": built a label for a row it will never fill")
            end
        ...
        assertEqual(
            ...
            subject.type
                ...
                .. tops[false]
                .. " and one that draws a row puts it at "
                .. tops[true]
        ...
        assertFont(
            fonts[false],
            fonts[true],
            subject.type .. ": a panel drawing no supporting row reads at a different size from" .. " one that does"
        )
    end
end
...
local function testReadingsSitInTheirSlots()
    ...
    local SLOTTED = {
        ...
        },
        {
            ...
            visual = function(panel)
                return panel.glyphShown and panel.glyph and { x = panel.glyph.x, w = panel.glyph.width } or nil
            end,
        },
        {
            type = "navigation",
            ...
            variants = { {} },
            visual = function(panel)
                if not (panel.showCompass and panel.compass) then
                    return nil
                end
            ...
            rows = function(panel)
                ...
                if panel.showCoordinates then
                    found[#found + 1] = { label = panel.coordinatesLabel, slot = "whole" }
                end
                return found
            end,
        },
    ...
    for _, subject in ipairs(SLOTTED) do
        ...
        for _, case in ipairs(cases) do
            ...
            local context = createLoaded({ x = 0, y = 0, w = WIDTH, h = HEIGHT }, DEFAULT_OPTIONS, widgetPath)
            assertEqual(#context.errors, 0, table.concat(context.errors, "\n"))
            settle(context, 30)

            local panel = entryById(context, "pack").instance
            local where = subject.type .. " " .. span .. (extra[1] and (" with " .. extra[1]) or "")

            local reading = panel.value.properties
            local text = tostring(reading.text)
            assert(text ~= "" and text ~= "--", where .. ": no reading was drawn, so there is nothing to place")
            ...
            assert(
                ...
                string.format(
                    ...
                    centre - expected,
                    expected,
                    100 * centre / panelWidth,
                    100 * expected / panelWidth
                )
            )
            ...
            if panel.unit and not panel.unit.hidden then
                ...
                if type(unitFont) == "function" then
                    unitFont = unitFont()
                end
                assertEqual(
...
local function testHostDiagnosticsReportsTheHost()
    ...
    assert(
        string.find(identity, context.layoutPath, 1, true),
        "the panel does not show the layout the host opened: " .. identity
    )
    assertEqual(context.layoutOrigin, "default", "this widget has no model-specific layout, so the fallback answered")
    assert(
        string.find(identity, "-> " .. context.layoutOrigin, 1, true),
        "the panel does not say which candidate name answered: " .. identity
    )
    assert(string.find(identity, "main.lua ", 1, true), "the panel does not stamp the running source: " .. identity)
    -- The stamp is the answer to stale bytecode, so it has to be a reading of
    -- the file rather than a placeholder.
    assert(string.find(identity, "2026%-09%-18"), "the panel shows no modification time: " .. identity)
    ...
    assert(
        not string.find(identity, "RUNNING main.luac", 1, true),
    ...
    assert(
        string.find(theme, "asked by: " .. context.themeSource, 1, true),
    ...
    assert(
        string.find(sources, "sources, ", 1, true) or string.find(sources, "no sources subscribed", 1, true),
    ...
    assert(panel.rows[panel.visibleLines + 1].hidden, "a line beyond what fits was left visible")
...
components:
  - id: identity
    ...
    assert(
        not string.find(identityLines(clean), "RUNNING main.luac", 1, true),
    ...
    assert(
        string.find(warned, "RUNNING main.luac", 1, true),
...
return latebreak
    ...
    assert(string.find(wires, "1 unbound", 1, true), "the panel does not count the unbound source: " .. wires)
    assert(
        string.find(wires, "NoSuchSensor UNBOUND", 1, true),
...
components:
  ...
  - id: quiet
    ...
    assertEqual(
        tall.detailLabel.properties.text,
        "#1",
    ...
    assertEqual(quiet.rendered.detail, nil, "a panel that never asked for the mode number computed one")
    assertEqual(quiet.detailLabel, nil, "a panel that never asked for the mode number built a label for it")
    assertEqual(quiet.showDetail, false, "a panel with no mode number to show reserved a row for one")
    ...
    assert(
        themeModule.measureText(font, widest) <= tall.area.valueBudget,
        "the widest mode name is drawn past the panel: needs "
            .. themeModule.measureText(font, widest)
            .. " of "
    ...
    assertEqual(tall.showDetail, false, "a shortened panel kept a supporting row it has no space for")
    assertEqual(tall.rendered.detail, nil, "a shed mode number was still being formatted every frame")
    ...
    radio.flightMode = 2
    settle(context, 20)
    assertEqual(tall.detailLabel.writes, writes, "a shed row was painted while the mode changed underneath it")
...
components:
  - id: mode
    ...
    local function fontFor(names)
        ...
        local context =
            createLoaded({ x = 0, y = 0, w = 480, h = 272 }, DEFAULT_OPTIONS, makeWidget("fm-sizing", layout))
        assertEqual(#context.errors, 0, table.concat(context.errors, "\n"))
        settle(context, 20)
    ...
    assertEqual(shortWidest, "Down", "the panel did not take its widest name from the model")
    assertEqual(longWidest, "LongRange7")
    assert(
        themeModule.fontHeight(shortFont) > themeModule.fontHeight(longFont),
        "a model whose mode names are short gained nothing: both readings are " .. edgetx.fontName(shortFont)
    )
    resetRadio()
end
...
components:
  ...
  - id: half
    ...
    assertEqual(
        quiet.detailLabel.properties.text,
        "75% EST",
    ...
    assertEqual(
        quiet.barShown,
        true,
        "a panel that stated no range drew no bar, which is what this change" .. " exists to stop being the normal case"
    )
    ...
    assertEqual(half.detailLabel.properties.text, "75% EST", "a stated empty was mixed with the radio's full")

    -- An inverted range is not a range either, and falls back rather than
    -- producing a backwards fill.
    assertEqual(inverted.detailLabel.properties.text, "75% EST", "an inverted range was treated as usable")
    ...
    assertEqual(
        stated.detailLabel.properties.text,
        "72% EST",
...
components:
  ...
  - id: tiny
    ...
    assertEqual(glyph.fill.properties.h, expected, "the level is not the voltage's share of the battery's interior")
    assertEqual(
        glyph.fill.properties.y,
    ...
    assert(
        expected > 0 and expected < glyph.interiorHeight,
    ...
    assert(glyph.fill.properties.y >= glyph.bodyY + glyph.border, "the level starts on or above the outline")
    assert(
        glyph.fill.properties.y + glyph.fill.properties.h <= glyph.bodyY + glyph.bodyHeight - glyph.border,
        "a full level would paint over the outline's bottom edge"
    )
    assert(glyph.fill.properties.w < glyph.width, "the level is as wide as the outline, so it covers the sides")
    ...
    assertEqual(
        glyph.shell.painted.borderWidth,
        glyph.border,
        "the outline reached LVGL at a different weight from the one asked for"
    )
    assert(glyph.border >= 2, "the outline is a hairline rather than the drawn cell the mock shows")
    ...
    assert(glyph.nub.properties.y + glyph.nub.properties.h <= glyph.bodyY, "the terminal overlaps the body it sits on")
    assert(glyph.nub.properties.w < glyph.width, "the terminal is as wide as the battery, so it reads as a cap")
    ...
    for name, part in pairs({
        outline = alarmed.glyph.shell,
        terminal = alarmed.glyph.nub,
        level = alarmed.glyph.fill,
    }) do
        assertEqual(
    ...
    assert(
        healthy.glyph.fill.properties.color ~= alarmed.glyph.fill.properties.color,
    ...
    -- `LcdFlags` words.
    assert(
        themeModule.contrast(context.theme.rgb.critical, context.theme.alertRgb.critical) >= 3,
    ...
    for name, part in pairs({ shell = glyph.shell, nub = glyph.nub, fill = glyph.fill }) do
        assertEqual(part.hidden, false, "the battery was revealed and its " .. name .. " was left hidden")
    end
    ...
    assertEqual(tiny.glyph, nil, "a single cell drew a battery in space it does not have")
...
components:
  ...
  - id: large
    ...
    assert(
        themeModule.fontHeight(smallFont) < themeModule.fontHeight(largeFont),
    ...
    assert(
        primitivesModule.batteryStroke(themeModule, smallFont, held)
    ...
    assert(
        small.glyph.border < large.glyph.border,
        "the same stroke ("
            .. small.glyph.border
            .. ") was drawn beside a "
            .. edgetx.fontName(smallFont)
            .. " reading and a "
            .. edgetx.fontName(largeFont)
            .. " one"
    )
    ...
    assert(
        small.glyph.shell.painted.borderWidth < large.glyph.shell.painted.borderWidth,
    ...
    local lightInterior = held - 2 * primitivesModule.batteryStroke(themeModule, smallFont, held)
    local heavyInterior = held - 2 * primitivesModule.batteryStroke(themeModule, largeFont, held)
    assert(lightInterior > heavyInterior, "the lighter outline bought the level no room")
    ...
    for id, panel in pairs({ small = small, large = large }) do
        ...
        assert(
            level > 0 and level < panel.glyph.interiorHeight,
            id .. " draws a full or empty cell, so its level says nothing"
        )
        assert(
            panel.glyph.interiorWidth >= 4,
            id .. " has a " .. panel.glyph.interiorWidth .. " pixel interior, which is a line rather than a level"
        )
    end
...
components:
  - id: pack
    ...
    -- pinning the supporting row moved the cap the reading is sized against.
    -- This panel draws no row at these heights, so what moved it is the cap
    -- rather than the row. Each time
    ...
    assert(
        themeModule.fontHeight(pack.value.properties.font()) < themeModule.fontHeight(builtFont),
    ...
    assertEqual(glyph.width, 25, "the cell changed size, so the stroke would have been free to change")
    ...
    assertEqual(
        glyph.shell.painted.borderWidth,
        builtStroke,
        "the mock let a border width through after build, which the firmware" .. " does not"
    )
...
components:
  - id: tx
    ...
    assertEqual(
        narrowGap,
        themeModule.unitGap(tx.unit.properties.font()),
        "the unit does not begin where the reading ends plus the stated gap"
    )
    assert(
        narrowGap <= 3,
        "the unit sits " .. narrowGap .. " pixels from the number, which is a gap rather than a rider"
    )

    -- Four characters. The reading grows and the unit has to grow with it.
    radio.values[320] = 10.0
    settle(context, 30)
    assertEqual(tx.text, "10.0", "the reading did not change length, so this test proves nothing")
    assert(
        tx.unit.properties.x > narrowX,
        "the reading grew a character and"
            .. " the unit stayed at "
            .. tx.unit.properties.x
            .. ", which leaves it sitting inside the number"
    )
    assertEqual(unitGap(), narrowGap, "the unit followed the reading but not by the width it gained")
    ...
    assertEqual(
        tx.unit.properties.x,
        narrowX,
        "the reading shrank and the unit stayed out at " .. tx.unit.properties.x .. ", leaving a gap"
    )
...
local function testUnitsRideBesideEveryReading()
    ...
    local function layoutAt(colSpan, rowSpan, from, count)
        ...
        local panels = {
            ...
            { id = "nav", type = "navigation", config = "      source: GPS\n      presentation: distance\n" },
    ...
    for _, span in ipairs({ { 2, 2, 1 }, { 2, 2, 4 }, { 2, 1, 1 }, { 2, 1, 4 } }) do
        ...
        local widgetPath = makeWidget("units-" .. span[1] .. "x" .. span[2] .. "-" .. span[3], text)
        local context = createLoaded({ x = 0, y = 0, w = 480, h = 272 }, DEFAULT_OPTIONS, widgetPath)
        assertEqual(#context.errors, 0, table.concat(context.errors, "\n"))
        settle(context, 60)
        ...
        for _, panel in ipairs(panels) do
            ...
            if not instance.unit.hidden and unit ~= "" then
                ...
                assert(
                    themeModule.fontHeight(unitFont) < themeModule.fontHeight(readingFont),
                    where
                        .. " draws its unit at "
                        .. edgetx.fontName(unitFont)
                ...
                assert(
                    not string.find(reading, unit, 1, true),
                    where .. " prints its unit inside the reading (" .. reading .. ") as well as beside it"
                )
                ...
                local drawnRight = readingLabel.properties.x + lcd.sizeText(reading, readingFont)
                assertEqual(
                    instance.unit.properties.x - drawnRight,
                    themeModule.unitGap(unitFont),
                    where .. " places its unit against an estimate rather than against" .. " where the reading is drawn"
                )
            end
        end
    end
    ...
    local sizes = 0
    for _ in pairs(seenFonts) do
        sizes = sizes + 1
    end
    assert(
        sizes >= 2,
        "every panel resolved the same reading font, so this" .. " test says nothing about a rule that varies with size"
    )
    assert(checked >= 8, "only " .. checked .. " panels actually drew a unit to check")
end
...
components:
  - id: pack
    ...
    reflow(140, 140)
    assertEqual(pack.glyphShown, false, "a panel with no room for a battery kept drawing one")
    for name, part in pairs({ shell = glyph.shell, nub = glyph.nub, fill = glyph.fill }) do
        assertEqual(part.hidden, true, "the battery was shed and its " .. name .. " stayed on screen")
    end
    ...
    for name, part in pairs({ shell = glyph.shell, nub = glyph.nub, fill = glyph.fill }) do
        assertEqual(part.hidden, false, "the battery came back and its " .. name .. " stayed hidden")
    end
    assertEqual(
        glyph.fill.properties.h,
        shownWidth,
        "the battery came back with a different level from the one it left with"
    )
    assertEqual(glyph.nub.properties.y, glyph.y, "the terminal came back detached from the body")
...
local function testBatteryGlyphLeavesTheReadingRoom()
    ...
    for index = 1, #spans do
        ...
        if span == "1x2" then
            assertEqual(glyph, nil, span .. " drew a battery beside a reading that fills the panel")
        else
            ...
            assert(
                ...
                "panel "
                    ...
                    .. " puts its battery at "
                    .. glyph.x
                    .. ", inside a reading column that ends at "
                    .. right
            )
        end
        ...
        assertEqual(
            panel.value.lines,
            1,
            "panel " .. index .. " wrapped its reading into " .. tostring(panel.value.lines) .. " lines"
        )
    end
...
components:
  - id: quiet
    ...
    assertEqual(quiet.barShown, false, "a bar was drawn with nothing to measure it against")
    assertEqual(quiet.detailLabel.properties.text, "", "a percentage was estimated from nothing")
    assertEqual(#context.errors, 0, table.concat(context.errors, "\n"))
    resetRadio()
end
...
components:
  ...
  - id: flat
    ...
    assertEqual(flat.rendered.detail, nil, "a shed caption was still being formatted every frame")
    ...
    settle(context, 20)
    assertEqual(flat.detailLabel.writes, writes, "a shed caption was written while the timer changed underneath it")
    resetRadio()
end
...
components:
  - id: clock
    ...
    assert(
        formWidth <= budget,
        "the form itself does not fit the panel it" .. " sized: " .. formWidth .. " in " .. budget
    )
    ...
    for _, case in ipairs(cases) do
        ...
        assertEqual(clock.value.properties.text, case[2], case[3] .. ": what was computed is not what was drawn")
        ...
        assert(
            width <= formWidth,
            case[3] .. " drew " .. width .. " px, wider than the " .. formWidth .. " px form its font came from"
        )
    ...
    assertEqual(
        widest,
        formWidth,
        "no reading in the sweep was as wide as the form, so this proves only" .. " that narrow strings fit"
    )
...
local function testReconcileBar()
    ...
    assertEqual(
        bar.marker.hidden,
        false,
        "a bar's marker was left behind, which is what reconciling its two" .. " other objects separately does"
    )
    ...
    assertEqual(bar.track.visibilityCalls, calls, "a bar whose visibility had not moved was told what it already was")
...
local function testUnitNeedsAValueToQualify()
    ...
    local context = {
        ...
        unit = primitivesModule.unit(root, theme, { x = 0, y = 0, font = TINSIZE, text = "V" }),
        showUnit = true,
        unitText = "V",
    }
    ...
    assertEqual(
        context.unit.hidden,
        false,
        "a reading of zero lost its unit, so the rule read availability rather" .. " than what the panel draws"
    )
    ...
    assert(bare.value.properties.w < pairWidth, "the drawn box did not shrink, so the unit's width is still reserved")
    ...
    place("N/A")
    assertEqual(context.unit.writes, writes, "a hidden unit was re-placed against a reading nobody can see it beside")
...
local function testHeadingNeverWraps()
    ...
    assert(
        overflowing.drawnHeight > themeModule.fontHeight(firmware.SMLSIZE),
        "a wrapped label did not grow, so the growth this exists to prevent" .. " cannot be seen either"
    )
    box:clear()
...
components:
  ...
  - id: huge
    ...
    assert(column > 0 and column < 60, "a single cell's heading column should be narrow: " .. column)
    ...
    for id, panel in pairs({ short = short, long = long, huge = huge }) do
        assertEqual(
            panel.label.lines,
            1,
            id .. "'s heading wrapped onto " .. tostring(panel.label.lines) .. " lines and grew down over the reading"
        )
        assert(
            panel.label.drawnHeight <= themeModule.fontHeight(panel.fonts.label),
    ...
    assertEqual(
        long.label.properties.text,
        "BATTERY",
        "a heading was cut when a smaller font would have carried it whole"
    )
    assert(
        themeModule.fontHeight(long.label.properties.font()) < themeModule.fontHeight(long.fonts.label),
    ...
    assert(
        #huge.label.properties.text < #"TRANSMITTER BATTERY PACK",
    ...
    local function noticedHeading(id)
        for _, notice in ipairs(context.notices) do
            if string.find(notice.text, "heading", 1, true) and string.find(notice.text, id .. ":", 1, true) then
                return notice.text
            end
        end
        return nil
    ...
    assertEqual(noticedHeading("short"), nil, "a heading that fitted was reported as cut")
    assertEqual(noticedHeading("long"), nil, "a heading carried whole by a smaller font was reported as cut")

    local said = false
    for _, notice in ipairs(context.notices) do
        if
            string.find(notice.text, "TRANSMITTER BATTERY PACK", 1, true)
            and string.find(notice.text, "huge", 1, true)
        then
            said = true
        end
    end
    assert(said, "a heading was cut and nobody was told; notices were: " .. table.concat(
        (function()
            local out = {}
            for _, notice in ipairs(context.notices) do
                out[#out + 1] = notice.text
            end
            return out
        end)(),
        " | "
...
components:
  - id: panel
    ...
    assertEqual(panel.label.lines, 1, "the heading wrapped after a reflow narrowed its column")
    assert(
        themeModule.fontHeight(panel.label.properties.font()) < themeModule.fontHeight(panel.fonts.label),
...
components:
  ...
  - id: nav
    ...
    assertEqual(cells.countLabel.properties.text, "5S", "a revealed cell count still reports the pack it was shed with")
    -- With quality leading, the supporting row names RSSI, which fell from
    -- -70 to -95 while the row was not on screen.
    assert(
        string.find(link.linkLabel.properties.text, "-95", 1, true),
        "a revealed link row still reports the RSSI it was shed with: " .. tostring(link.linkLabel.properties.text)
    )
    -- The vertical speed reversed while the row was not on screen.
    assert(
        string.find(alt.secondary.properties.text, "-3", 1, true),
        "a revealed secondary row still reports the value it was shed with: " .. tostring(alt.secondary.properties.text)
    )
    ...
    assertEqual(
        cells.countLabel.properties.text,
        cells.countText,
...
components:
  ...
  - id: tight
    ...
    assertEqual(trims.showCaption, false, "a narrowed trim panel kept captions it has no room for")
    assertEqual(trims.showValue, false, "a narrowed trim panel kept readouts it has no room for")
    ...
    assertEqual(trims.rendered.text1, nil, "a shed readout was still being formatted every frame")
    assertEqual(
        trims.rendered.fraction1 ~= nil,
        true,
        "the bar stopped being declared, which is the part that is still drawn"
    )
    assertEqual(trims.indicators[1].valueText, "", "a shed readout left its last wording behind as if still current")
    ...
    assertEqual(
        trims.indicators[1].caption.writes,
        captionWrites,
        "a reflow repositioned a caption the panel had already shed"
    )
    assertEqual(
        trims.indicators[1].value.writes,
        valueWrites,
        "a reflow repositioned a readout the panel had already shed"
    )
    assertEqual(
        trims.indicators[1].caption.visibilityCalls,
        captionCalls,
        "a reflow hid a caption that was already hidden"
    )
    assertEqual(trims.indicators[1].value.visibilityCalls, valueCalls, "a reflow hid a readout that was already hidden")
    ...
    assert(
        not tight.unit.hidden,
        "the metric shed a unit it has room for, so the line below is measuring" .. " a hidden object"
    )
    assertEqual(
        tight.unit.writes,
        unitWrites + 1,
        "a reflow moved the unit more than once, or not at all: it moved "
            .. (tight.unit.writes - unitWrites)
            .. " times"
    )
    zone.w = 240
    passes = 0
    ...
    assertEqual(trims.indicators[1].caption.properties.text, "AIL", "a revealed caption came back blank")
    assertEqual(trims.indicators[1].caption.hidden, false)
    assertEqual(
    ...
    assertEqual(trims.showCaption, true, "a mild narrowing shed captions that still fit")
    assert(
        trims.indicators[2].caption.properties.x < captionBefore,
        "a narrowed panel left its captions at their old positions"
    )
    assertEqual(
        trims.indicators[2].caption.hidden,
        false,
        "a caption that never changed visibility was hidden by the reflow"
    )
    assertEqual(
        trims.indicators[1].caption.properties.text,
        "AIL",
    ...
    local narrow = createLoaded(narrowZone, DEFAULT_OPTIONS, makeWidget("trim-shed-narrow", layout))
    ...
    assertEqual(
        born.indicators[1].caption.properties.text,
        "",
...
local function testCoreComponents()
    ...
    assertEqual(countdown.showDetail, false, "a one-row timer found space for a supporting row")
    ...
    assertEqual(countdown.rendered.detail, nil, "a shed caption was still being formatted every frame")
    ...
    assertEqual(mode.rendered.detail, nil, "a shed mode number was still being formatted every frame")
    ...
    assertEqual(mode.showDetail, false, "a single-cell flight mode found room for a supporting row")
    ...
    assertEqual(mode.detailLabel, nil, "a panel that never asked for the mode number built a label for it")
    ...
    assertEqual(battery.showDetail, false, "a single-cell battery found room for a percentage")
    assertEqual(battery.rendered.detail, nil, "a shed percentage was still being formatted every frame")
    ...
    assertEqual(trims.showCaption, false, "a one-row trim panel found room for captions")
    assert(trims.indicators[1].caption.hidden, "a shed caption was left on screen")
    -- A positive trim fills rightward from the centre of its own bar.
    local fill = trims.indicators[1].bar.fill.properties
    assert(
        fill.x >= trims.indicators[1].bar.x + math.floor(trims.indicators[1].bar.w / 2),
    ...
    assertEqual(
        identity.image.properties.fill,
        false,
        "the model picture is cropped to fill its frame instead of fitted" .. " inside it"
    )
    ...
    assertEqual(
        identity.label.properties.text,
        "TEST MODEL",
...
local function testUnitsAreNotDrawnBesideAnAbsentReading()
                ...
                if componentHost.supportsSpan(module, colSpan, rowSpan) then
                    ...
                    if entry then
                        ...
                        if unit and (instance.unitText or "") ~= "" then
                            ...
                            assert(
                                primitivesModule.SENTINELS[instance.text],
                                where
                                    .. " prints `"
                                    .. tostring(instance.text)
                                    .. "` for a"
    ...
    for _, stem in ipairs({ "tx-battery", "cell-battery", "link-status", "metric" }) do
        assert(
            constructed[stem],
            stem
                .. " never reached a panel that knew its"
...
components:
  - id: pack
    ...
    assertEqual(instance.unitText, "V", "this test needs a panel that knows its unit without being told")
    ...
    reflow()
    assert(
        instance.area.showUnit,
        "this step must keep the unit permitted, or it is not testing the" .. " short-circuit it was written for"
    )
    assertEqual(instance.unit.hidden, true, "a reflow put the unit back beside a reading that still has no value")
    ...
    assertEqual(
        instance.area.showUnit,
        false,
        "this step must withdraw the unit's permission, or the widening below" .. " restores nothing and proves nothing"
    )
    ...
    assertEqual(instance.unit.hidden, true, "growing the panel restored a unit to a reading that still has no value")
...
components:
  ...
  - id: battery
    ...
    local function assertUnavailable(id)
        ...
        assertEqual(instance.badge.properties.text, "N/A", id .. " reported its state by colour alone")
    end
    ...
    local identity = entryById(context, "identity").instance
    assert(identity.area.showImage, "the degradation test must run on a panel that could show an image")
...
local function testCoreComponentsReflow()
    ...
    local function assertContained(what)
        for _, entry in ipairs(context.components) do
            ...
            assertEqual(panel.h, bounds.h, what .. ": " .. entry.placement.id .. " did not follow its container height")
...
local function testTelemetryComponents()
    ...
    assertEqual(elrs.unit.properties.text, "dBm", "the unit the source resolved to never reached the label beside it")
...
components:
  - id: nav
    ...
    assertEqual(nav.compass.ring.hidden, true, "the dial kept its room on a panel whose distance needed it")

    zone.w = 480
    drain()
    pump(context, 10)
    assertEqual(nav.compass.ring.hidden, false, "the dial was not restored")
    assertEqual(nav.compass.ring.properties.radius, firstRadius, "the dial was not restored to its full size")
...
components:
  - id: nav
    ...
    assertEqual(nav.showCompass, true, "the panel shed its compass, so this test proves nothing")
...
local function testTelemetryDegrades()
    ...
    assert(
        themeModule.measureText(pack.fonts.label, pack.countText) <= pack.detailWidth,
        "the cell-count row overran its box"
    )
...
components:
  ...
  - id: absent
    ...
    for _, id in ipairs({ "extreme", "voltage" }) do
        ...
        assertEqual(instance.countText, "CELLS ERR", id .. " reported a shape problem as a missing source")
...
components:
  - id: nav
    ...
    assertEqual(nav.stateName, "unavailable")
    assertEqual(nav.badge.properties.text, "N/A", "a sensor that appeared was still reported as a missing source")
    assertEqual(nav.origin, "NO FIX")
...
components:
  ...
  - id: rssionly
    ...
    local link = entryById(context, "link").instance
    assertEqual(link.primaryName, "quality", "auto must fall back to the source the protocol actually has")
